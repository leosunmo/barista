...
func TestInvalid(t *testing.T) {
	...
	err := afero.WriteFile(fs, "/src/material-error-1/font/MaterialIcons-Regular.codepoints", []byte(
	...
	err = afero.WriteFile(fs, "/src/material-error-3/font/MaterialIcons-Regular.codepoints", []byte(
		`someIcon 61
		 otherIcon 62
		 badIcon xy`,
	), 0644)
...
func TestValid(t *testing.T) {
	fs = afero.NewMemMapFs()
	err := afero.WriteFile(fs, "/src/material/font/MaterialIcons-Regular.codepoints", []byte(
		`someIcon 61
		 otherIcon 62
		 thirdIcon 63`,
	), 0644)
